...
| 기존 문제점                                                              |
|:--------------------------------------------------------------------|
| - 모임 정보에 대한 분산<br/> - 신원 불명, 후기 시스템 부재 <br/> - 모임 이후 지속적 관계 유지의 어려움 |
...
> ⛔ **Non-Retryable 예외**  
> → 역직렬화 실패·검증 실패 등은 즉시 DLT  
> → 지연 최소화 & 자원 절약

> 👀 **관측 가능성 확보**  
> → DLT 전송 시 `log.error` 기록  
> → 문제 원인 추적 가능

---
### 🛰️ KRaft를 통한 클러스터 환경 구축
...
### 도입 배경과 필요성
**[문제 상황]**
<br />❌ 게시글의 데이터 규모가 증가할수록 성능 저하 ➡ 대용량 데이터 처리 시 인덱스 관리와 I/O 부담이 커져 성능 저하 발생
<br />❌ 게시글 조건 검색과 페이징 처리의 빈도수가 높음 ➡ 복잡한 검색 조건과 빈번한 페이징 요청이 DB 부하를 가중시켜 조회 성능 저하
...
NestedQuery + TermQuery ➡ BoolQuery ➡ NativeQuery 순서로 조합
...
## 👨‍👩‍👧‍👦 **팀 구성원 소개**
| **구성원**  |**사진**| **직책**  | **장점**                                       | **GitHub**                                | **Blog**                                        | **담당**                                                                                                     |
|----------|---|---------|----------------------------------------------|-------------------------------------------|-------------------------------------------------|------------------------------------------------------------------------------------------------------------|
| **김민균**  |![KakaoTalk_20231129_122808031](https://github.com/user-attachments/assets/73948fe8-168c-4f0f-a575-e51a2b0699a3)| `👑팀장`  | `🎨아이디어 뱅크`<br> `❄️냉철한 시선`<br> `😄즐거움의 달인`   | [GitHub](https://github.com/MinGyunKim27) | [블로그](https://velog.io/notifications)           | 📌 Kafka 시각화(모니터링)<br>📌 배포(CI/CD) <br> 📌 기술스택, 취미 기능 구현 <br>📌 유저 팔로우 구현                                 |
